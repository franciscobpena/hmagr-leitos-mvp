...
/**
 * Mapeia `giro_motivo` (GIRO_MOTIVOS, decidido por um humano em kfConfirmarGiro/index.html)
 * para o vocabulário real de `internacoes_hmsa.status_internacao` já em uso em produção por
 * `_onda6ConfirmarOutrasSaidas` (index.html:14827-14834): alta_medica|transferido_externo|
 * desospitalizacao|obito|evasao|cancelada. Corrige achado S2 do review (25/jun): o mapeamento
 * anterior colapsava tudo em só 2 valores ('alta_medica'/'cancelada'), perdendo a distinção
 * visual que STATUS_LABEL/statusOpts já fazem pros outros 4 status.
 */
function mapGiroMotivoToStatus(motivo) {
  const map = {
    alta: 'alta_medica',
    transferencia: 'transferido_externo',
    evasao: 'evasao',
    obito: 'obito',
    duplicado: 'cancelada'
  };
  return map[motivo] || 'cancelada'; // motivo não previsto: mesmo fallback conservador de antes
}
